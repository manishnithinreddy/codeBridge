...
echo "Testing Redis connection to 223.187.54.126:6379..."
if command -v redis-cli &> /dev/null; then
    if redis-cli -h 223.187.54.126 -p 6379 ping > /dev/null 2>&1; then
    ...
    if nc -zv 223.187.54.126 6379 2>/dev/null; then
...
echo "Testing PostgreSQL connection to 223.187.54.126:5432..."
if command -v pg_isready &> /dev/null; then
    if pg_isready -h 223.187.54.126 -p 5432 > /dev/null 2>&1; then
    ...
    if nc -zv 223.187.54.126 5432 2>/dev/null; then
